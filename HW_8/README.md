...
The result of the utility should be a list of full paths of files with identical contents, output to standard output. One file per line. Identical files should be in a row, in one group. Different groups are separated by an empty line.
...
ld\n\0\0
```
Each block must be collapsed by the chosen hashing function. Possible collisions are ignored. Assuming that
```
H("Hello") == A
H(", Wor") == B
H("ld\n\0\0") == C
