...
It is necessary to implement these functions in such a way as to preserve the previous functionality of the project.

The implementation must allow multiple calls to **connect()**. Calls to **receive()** with different contexts must not interfere with each other. Calls can be made from different threads, but calls with the same context are always made from the same thread. Optionally, implement the ability to call all functions from any thread.
...
The task is considered successful if, after installing the package, linking with the test code (example in **main.cpp**) and running with test data, the output matches the description of [**Task 6**](../HW_7/README.md). The data is supplied in portions in different contexts in large volumes without pauses.
