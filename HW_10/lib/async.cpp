...
void file_worker(const int thread_id) {
    ...
    while (true) {
        ...
        auto timestamp_seconds = std::chrono::floor<std::chrono::seconds>(
            task.get_timestamp().time_since_epoch()).count();

        const std::string filename = std::format("{}{}_{}_{}_{}.log", TASK_MANAGER_NAME(),
            timestamp_seconds, task.get_context_id(), thread_id, file_counter++);
