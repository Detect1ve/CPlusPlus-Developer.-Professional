    ...
    */
    ...
    public:
        ...
        */
        bool push(
            const T&                        item,
            const std::chrono::milliseconds timeout = -1ms)
        {
    ...
    private:
        ...
        */
         ...
         *         or std::nullopt if the queue is empty or closed
         */
        std::optional<T> popImpl(const std::chrono::milliseconds timeout)
        {
            bool success = false;
            std::unique_lock<std::mutex> lock(mutex_);
            T item;
            ...
            const std::chrono::milliseconds timeout)
        {
               ...
               && !closed_)
            {
                ...
                else if (timeout > 0ms)
                {
                    ...
                    success = notEmpty_.wait_for(lock, timeout, predicate);
                }
                else
                {
            ...
            const std::chrono::milliseconds timeout)
        {
               ...
               && !closed_)
            {
                ...
                if (timeout > 0ms)
                {
                    ...
                    return notFull_.wait_for(lock, timeout, predicate);
                }
